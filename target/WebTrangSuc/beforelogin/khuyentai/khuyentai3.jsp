
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <title>JB1583</title>
        <link rel="stylesheet" href="<%= request.getContextPath() %>/static/UserCss/product.css"/>
    </head>
    <body>
        <%@ include file="../../user/layout/header.jsp"%>
        <%@ include file="../../user/layout/navbar.jsp"%>
        <!--    Phần nội dung-->
<section class="section main">
    <div class="container">
        <div class="content">
            <img src="<%= request.getContextPath() %>/static/img/Home/khuyentai3.jpg" alt="#" class="img-prd"/>
        </div>

        <div class="sidebar">
            <div class="title">
                <h1 class="content-product-h3">Water Buffalo Horn Earrings - JB1583</h1>
            </div>
            <div class="discription">

                <h6>Not gold cuff bracelet, this Dasmoto quintuple cuff bracelet uses superior horn material for a distinctive and mysterious look. The item is covered by a solid dark color. Wear it on their own or mix and match with Jasmino necklaces and earrings to get the best performance. All materials are hypoallergenic and eco-friendly.</h6>
            </div>
            <div class="product-detail">
                <ul>
                    <li><strong>Trademark</strong>: DASMOTO</li>
                    <li><strong>Product'Name</strong>: JB1583</li>
                    <li><strong>Material</strong>: Superior buffalo horn</li>
                    <li><strong>Made in</strong>: Viet Nam</li>
                    <li><strong>Made by</strong>: Skilled craftsman</li>
                    <li><strong>Insurance</strong>: 6 months</li>
                </ul>
            </div>

            <div class="price">
                <span class="money"><strong>28 $</strong></span>
            </div>
            <div>
                <button type="submit" class="btn-cart"><i class="fa-solid fa-cart-plus"></i>
                </button>
            </div>

        </div>
    </div>

</section>
<!--    Hết phần nội dung-->
        <%@ include file="../../user/layout/footer.jsp"%>
    </body>
</html>
